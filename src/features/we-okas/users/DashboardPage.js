          ...
          <>
            ...
            >
              {confirmDelete ? (
                ...
                <>
                  <motion.button onClick={() => { toggle(false); onEdit(); }} whileHover={{ x: 4, backgroundColor: '#f4f7fb' }} className="w-full px-[16px] py-[12px] text-left text-[14px] text-[#0a1e3f] transition-colors font-['Inter:Medium',sans-serif] font-medium">Edit</motion.button>
                  <div className="h-px w-full bg-[#e2e2e2]" />
                  <motion.button whileHover={{ x: 4, backgroundColor: '#f4f7fb' }} className="w-full px-[16px] py-[12px] text-left text-[14px] text-[#0a1e3f] transition-colors font-['Inter:Medium',sans-serif] font-medium">Configure</motion.button>
                  <div className="h-px w-full bg-[#e2e2e2]" />
                  <motion.button onClick={() => setConfirmDelete(true)} whileHover={{ x: 4, backgroundColor: '#fef2f2' }} className="w-full px-[16px] py-[12px] text-left text-[14px] text-[#ff4444] transition-colors font-['Inter:Medium',sans-serif] font-medium">Delete</motion.button>
                </>
              )}
            </motion.div>
          </>
...
export default function UserDashboardPage() {
  ...
  const filterCategories = [
    ...
    { name: 'Manager', options: [...new Set(projects.map(p => p.manager))].sort().map(n => ({ label: n, value: n })) },
  ];
  ...
  const filteredProjects = projects.filter(project => {
    ...
    return (
      ...
      (selectedManagers.length === 0 || selectedManagers.includes(project.manager))
    );
  });
  ...
  const handleEdit = (id) => {
    const raw = apiProjects.find(p => p.id === id);
    if (raw) {
      setEditProjectData({
        buildingId: raw.building_id, buildingType: raw.building_type || 'Residential',
        address: raw.address, landmark: raw.landmark || '',
        assignedMember: raw.assigned_member, contactName: raw.name,
        phoneCountryCode: '+91', phoneNumber: raw.mobile || '',
        email: raw.email || '', serialNumber: raw.okas_box_info || '',
      });
      setEditProjectId(id);
  ...
  return (
    <div className="bg-white flex flex-col items-start relative h-screen w-full overflow-hidden">
      ...
      <div className="flex flex-1 items-start relative w-full overflow-hidden">
        ...
        <div className="bg-[#f4f7fb] flex flex-col gap-[28px] items-start overflow-y-auto pb-[40px] pt-[32px] px-[40px] relative flex-1 w-full h-full">
          <p className="font-['Inter:Semi_Bold',sans-serif] font-semibold leading-[1.1] text-[#0a1e3f] text-[40px] tracking-[-0.8px]">List of Projects</p>
            ...
            <div className="flex gap-[12px] items-center">
              ...
              <div className="bg-white h-[44px] rounded-[4px] w-[280px] border border-[#e2e2e2]">
                <div className="flex gap-[8px] items-center px-[16px] h-full">
                  ...
                  <input type="text" value={projectSearch} onChange={(e) => setProjectSearch(e.target.value)} placeholder="Search projects" className="flex-1 font-['Inter:Regular',sans-serif] text-[#5c7089] text-[14px] bg-transparent border-none outline-none placeholder:text-[#5c7089]" />
                </div>
              </div>
            </div>
          ...
          <div className="bg-white relative rounded-[4px] shrink-0 w-full border border-[#e2e2e2] overflow-x-auto">
      ...
      <AddProjectDrawer
        ...
        mode="edit" initialData={editProjectData} projectId={editProjectId}
      />
    </div>
  );
}
